...
//    }
    ...
    @ResponseBody
    @GetMapping(value = "/getAutoBlackList")
    public Response<?> getAutoBlackList(HttpServletRequest req) {
        try {
            JSONObject result = new JSONObject();
            AutoBlackListSetConf conf = PublicDataConf.centerSetConf != null ? PublicDataConf.centerSetConf.getAutoBlackList() : null;
            result.put("enabled", conf != null && conf.isEnabled());
            result.put("black_score", conf != null ? conf.getBlack_score() : -1);
            List<AutoBlackListSetConf.AutoBlackUser> records = HttpRoomData.loadAutoBlackRecords();
            ...
            LOGGER.error("getAutoBlackList error", e);
            return Response.success(null, req);
        }
    }

    @ResponseBody
    @PostMapping(value = "/saveAutoBlackList")
    public Response<?> saveAutoBlackList(@RequestParam("enabled") boolean enabled, @RequestParam("black_score") int blackScore, HttpServletRequest req) {
        try {
            if (PublicDataConf.centerSetConf != null) {
                AutoBlackListSetConf conf = PublicDataConf.centerSetConf.getAutoBlackList();
                if (conf == null) {
                    conf = new AutoBlackListSetConf();
                    PublicDataConf.centerSetConf.setAutoBlackList(conf);
                }
                conf.setEnabled(enabled);
                conf.setBlack_score(blackScore);
                checkService.changeSet(PublicDataConf.centerSetConf, false);
            }
            return Response.success(0, req);
        } catch (Exception e) {
            LOGGER.error("saveAutoBlackList error", e);
            return Response.success(1, req);
        }
    }

    @ResponseBody
    @GetMapping(value = "/auto_unblack")
    public Response<?> autoUnblack(@RequestParam("uid") long uid, HttpServletRequest req) {
        try {
            short code = -1;
            if (StringUtils.isNotBlank(PublicDataConf.USERCOOKIE)) {
                code = HttpUserData.httpPostDeleteBadList(uid);
            ...
            if (code == 0) {
                ...
                HttpRoomData.saveAutoBlackRecords(records);
            }
            return Response.success(code, req);
        } catch (Exception e) {
            LOGGER.error("autoUnblack error", e);
            return Response.success(1, req);
        }
    }

    @ResponseBody
    @GetMapping(value = "/auto_black_delete_display")
    public Response<?> autoBlackDeleteDisplay(@RequestParam("uid") long uid, HttpServletRequest req) {
        try {
            List<AutoBlackListSetConf.AutoBlackUser> records = HttpRoomData.loadAutoBlackRecords();
            records.removeIf(r -> r.getUid() != null && r.getUid().equals(uid));
            HttpRoomData.saveAutoBlackRecords(records);
            ...
            LOGGER.error("autoBlackDeleteDisplay error", e);
            return Response.success(1, req);
        }
    }
